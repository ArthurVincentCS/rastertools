...
# -*- coding: utf-8 -*-
"""
This module contains the geo-rastertools command line interface. The command line
has several subcommands such as *radioindice* and *zonalstats*.

Usage examples::

  geo-rastertools radioindice --help
  geo-rastertools zonalstats --help
...
import json
import click
from eolab.geo-rastertools.cli.filtering import filter
from eolab.geo-rastertools.cli.hillshade import hillshade
from eolab.geo-rastertools.cli.speed import speed
from eolab.geo-rastertools.cli.svf import svf
from eolab.geo-rastertools.cli.tiling import tiling
from eolab.geo-rastertools.cli.timeseries import timeseries
from eolab.geo-rastertools.cli.radioindice import radioindice
from eolab.geo-rastertools.cli.zonalstats import zonalstats
from eolab.geo-rastertools import __version__
from eolab.geo-rastertools.product import RasterType
...
@click.option(
    ...
    help="Maximum number of workers for parallel processing. If not given, it will default to "
            ...
            "run geo-rastertools, it is thus recommended to set this option.")
@click.option(
    '--debug',
    "keep_vrt",
    is_flag=True,
...
@click.version_option(version='geo-rastertools {}'.format(__version__))  # Ensure __version__ is defined
@click.pass_context

def rastertools(ctx, rastertype : str, max_workers : int, keep_vrt : bool, verbose : bool, very_verbose : bool):
    """
    Main entry point for the `geo-rastertools` Command Line Interface.

    The `geo-rastertools` CLI provides tools for raster processing and analysis and allows configurable data handling, parallel processing,
    ...
    Environment Variables:
        ...
        - `RASTERTOOLS_MAXWORKERS`: If `max_workers` is set, it defines the max workers for geo-rastertools.
    """
